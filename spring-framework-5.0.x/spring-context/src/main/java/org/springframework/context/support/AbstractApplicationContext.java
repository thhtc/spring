...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		...
		//refreshBeanFactory()在不同实现容器中有点区别，如果是以xml方式配置bean，
		// 会使用AbstractRefreshableApplicationContext容器中的实现，
		// 该容器中实现xml配置文件定位，并通过BeanDefinition载入和解析xml配置文件。
		//而如果是注解的方式，则并没有解析项目包下的注解，而是通过在refresh()方法中执行
		refreshBeanFactory();
		// 返回刚刚创建的 BeanFactory
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		//invokeBeanFactoryPostProcessors方法内部执行实现了BeanFactoryPostProcessor、BeanDefinitionRegistryPostProcessor这两个接口的Processor，
		// 先获取所有BeanDefinitionRegistryPostProcessor的实现，按优先级执行（是否实现PriorityOrdered优先级接口，是否实现Ordered顺序接口）；
		// 再以相同的策略执行所有BeanFactoryPostProcessor的实现。
	...
	protected void registerBeanPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		//注册BeanPostProcessor（Bean的后置处理器）,用于拦截bean创建过程. 注册后置处理器的大致逻辑是：
